...
export function CreateContribution({ accounts, onSave, onDone }) {
  ...
  async function handleSubmit() {
    ...
    if (response.error === undefined) {
      ...
      // Save contribution
      onSave(contribution)
      setCreated(accounts.find(acc => acc.getId() === selectedAccount).getName())
  ...
  return (
    <form onSubmit={handleSubmit}>
      {
        created !== undefined && <div className="alert alert-success">Successfully created contribution for account "{created}"</div>
      }
      {error !== undefined && (
      ...
      <div className="form-group">
        ...
        >
          ...
          {
            accounts.filter(acc => !alreadyCreated.includes(acc.getId())).map(function(account) {
              return <option key={account.getId()} value={account.getId()}>{account.getName()}</option>
            })
          }
        </select>
      </div>
